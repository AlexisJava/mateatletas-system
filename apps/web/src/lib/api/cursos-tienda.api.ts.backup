...
export const cursosTiendaApi = {
  ...
  obtenerCatalogo: async (filtros?: FiltrosCatalogo): Promise<CursoCatalogo[]> => {
    try {
      ...
      const response = await apiClient.get<CursoCatalogo[]>(url);
      return response as any; // Interceptor ya retorna response.data
    } catch (error) {
  ...
  obtenerMisCursos: async (): Promise<CursoEstudiante[]> => {
    try {
      const response = await apiClient.get<CursoEstudiante[]>('/gamificacion/tienda/mis-cursos');
      return response as any;
    } catch (error) {
