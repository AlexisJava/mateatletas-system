  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 bg-white/20 backdrop-blur-sm rounded-full px-3 py-1 border-2 border-white/30">
        <span className="text-white text-xs font-black">{progreso.progresoPorcentaje}%</span>
      </div>

      {/* Emoji grande */}
      <div className="text-6xl mb-4">{metadatos.emoji}</div>

      {/* Título */}
      <h3 className="text-2xl font-black text-white mb-2 uppercase">{metadatos.titulo}</h3>

      {/* Descripción */}
      <p className="text-white/90 text-sm font-medium mb-4 flex-1">{metadatos.descripcion}</p>
      ...
      <div className="flex items-center gap-1 mb-3">
        ...
        {Array.from({ length: progreso.estrellas }).map((_, i) => (
          <Star key={`filled-${i}`} className="w-5 h-5 fill-yellow-400 text-yellow-400" />
        ))}
        {/* Estrellas vacías */}
        {Array.from({ length: estrellasVacias }).map((_, i) => (
          <Star key={`empty-${i}`} className="w-5 h-5 text-white/30" />
        ))}
      </div>
      ...
      <div className="flex items-center justify-between text-white/80">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs font-black text-yellow-300">{progreso.puntosGanados} pts</span>
        </div>
      </div>
      ...
      {progreso.progresoPorcentaje === 100 && (
        <div className="absolute -top-2 -right-2 bg-gradient-to-r from-green-500 to-emerald-600 rounded-full px-4 py-2 border-3 border-white shadow-xl">
          <span className="text-white text-xs font-black uppercase">✓ Completada</span>
        </div>
      )}
    </motion.button>
  );
