...
describe('EstudiantesService - Copiar entre Sectores', () => {
  ...
  describe('RED - Test 8: Copiar estudiante existente a nuevo sector', () => {
    it('debería vincular un estudiante existente a un nuevo sector', async () => {
      ...
      jest.spyOn(prisma.estudiante, 'findUnique').mockResolvedValue(estudianteExistente as any);
      jest.spyOn(prisma.sector, 'findUnique').mockResolvedValue(nuevoSector as any);
      ...
      // Act
      const result = await service.copiarEstudianteASector(estudianteId, nuevoSectorId);
  ...
  describe('RED - Test 9: Validar que el sector destino existe', () => {
    it('debería lanzar BadRequestException si el sector no existe', async () => {
      ...
      jest.spyOn(prisma.estudiante, 'findUnique').mockResolvedValue({ id: estudianteId } as any);
      jest.spyOn(prisma.sector, 'findUnique').mockResolvedValue(null);

      // Act & Assert
      await expect(service.copiarEstudianteASector(estudianteId, sectorInexistente)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.copiarEstudianteASector(estudianteId, sectorInexistente)).rejects.toThrow(
  ...
  describe('RED - Test 10: Validar que el estudiante existe', () => {
    it('debería lanzar BadRequestException si el estudiante no existe', async () => {
      ...
      jest.spyOn(prisma.sector, 'findUnique').mockResolvedValue({ id: sectorId } as any);

      // Act & Assert
      await expect(service.copiarEstudianteASector(estudianteInexistente, sectorId)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.copiarEstudianteASector(estudianteInexistente, sectorId)).rejects.toThrow(
  ...
  describe('RED - Test 11: Prevenir duplicación en el mismo sector', () => {
    it('debería lanzar ConflictException si el estudiante ya está en ese sector', async () => {
      ...
      // Act & Assert
      await expect(service.copiarEstudianteASector(estudianteId, sectorId)).rejects.toThrow(ConflictException);
      await expect(service.copiarEstudianteASector(estudianteId, sectorId)).rejects.toThrow(
        'El estudiante ya está asignado a este sector',
      );
    });
  });
  ...
  describe('RED - Test 12: Buscar estudiante por DNI para copiar', () => {
    it('debería permitir buscar estudiante por DNI para copiarlo', async () => {
      ...
      // Act
      const result = await service.copiarEstudiantePorDNIASector(dni, nuevoSectorId);
